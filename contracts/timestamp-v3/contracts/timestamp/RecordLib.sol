...
library RecordLib {
    ...
    uint8 public constant MAX_TIMESTAMPS_PER_VERSION = 10;
    uint8 public constant MAX_RECORDS_PER_TIMESTAMP = 10;
    ...
    event RecordOwnerAdded(
        bytes32 recordId,
        string ownerId,
        uint256 notBefore,
        uint256 notAfter
    );
    event OwnerIdRevoked(bytes32 recordId, string ownerId, uint256 notAfter);
    ...
    function detachRecordVersionHash(
        ...
        for (uint256 i = 0; i < vd.timestampsIds.length; i++) {
            if (vd.timestampsIds[i] == timestampId) {
                ...
                emit TimestampIdDetached(timestampId, recordId, versionId);
                break;
            }
        }
    ...
    function insertRecordOwner(
        ...
        emit RecordOwnerAdded(recordId, ownerId, notBefore, notAfter);
    }
    ...
    function revokeRecordOwner(
        ...
        emit OwnerIdRevoked(recordId, ownerId, block.timestamp);
    }
