...
describe("Trusted Apps", () => {
  ...
  beforeEach(async () => {
    ...
    // insert public keys
    await tar.insertAppPublicKey(app.id, app.publicKey, 1);
    await tar.insertAppPublicKey(app2.id, app2.publicKey, 1);
    await tar.insertAppPublicKey(authApp.id, authApp.publicKey, 1);

    // authApp is authorized to use app
    await tar.insertAuthorization(app.name, authApp.name, app.admin, 1);
  ...
  it("should get an app by name", async () => {
    const appByName = await tar.getAppByName(app.name);
    expect({
      applicationId: appByName.applicationId,
      domain: appByName.domain,
    }).to.deep.equal({
  ...
  it("should reject no authenticated users", async () => {
    ...
    await expect(tar.updateAuthorization(firstAuthId, 2)).to.be.revertedWith(
      [
        "Policy error: sender is not controller of any of the ",
        "adminitrators of app 'my-app' and it doesn't have ",
  ...
  it("should prevent duplications for app admin", async () => {
    await policyContractMock.setPolicyResult(false);
    await expect(tar.insertAppAdministrator(app.id, "did:newadmin")).to.emit(
      tar,
      "ApplicationAdministratorAdded"
  ...
  it("should allow up to 10 (MAX_ADMINS) per app", async () => {
    await policyContractMock.setPolicyResult(true);
    await didContractMock.setDidResult(true);

    // eslint-disable-next-line no-plusplus
    for (let i = 0; i < MAX_ADMINS; i++) {
      if (i < MAX_ADMINS - 1) {
