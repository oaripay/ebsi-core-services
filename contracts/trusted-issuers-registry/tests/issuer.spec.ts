...
describe("Issuers", () => {
  ...
  let didV4ContractMock: Contract;
  ...
  before(async () => {
    ...
    didV4ContractMock = didRegistryFactory.attach(testDidrV4Address);
    await didContractMock.setDidResult(false);
    await didV4ContractMock.setDidResult(true);
  });
  ...
  describe("Entities", () => {
    it("should reject no authenticated users", async () => {
      ...
      await didV4ContractMock.setDidResult(false);
      await expect(
        tir.insertIssuer(
          didIssuer,
          attributeData1,
    ...
    it("should insert/update an issuer as admin", async () => {
      ...
      await didV4ContractMock.setDidResult(false); // the admin is not a controller
    ...
    it("should insert/update an issuer as TAO/RootTao", async () => {
      ...
      // remove admin
      await policyContractMock.setPolicyResult(false);
      await didV4ContractMock.setDidResult(true); // the admin is a controller
  ...
  describe("Attributes", () => {
    it("should update its own attributes only if it's a RootTAO", async () => {
      ...
      await didV4ContractMock.setDidResult(true); // is a controller
      // update: insert attribute
      await expect(
        tir
          .connect(issuer)
