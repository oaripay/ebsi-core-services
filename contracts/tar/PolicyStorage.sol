...
contract PolicyStorage {
    ...
    bytes32 public constant TAR_POLICY_DIAMOND_STORAGE_POSITION = keccak256(
        "diamond.standard.tar.policy.storage"
    );
    ...
    function policyStorage() internal pure returns (Policies storage ms) {
        bytes32 position = TAR_POLICY_DIAMOND_STORAGE_POSITION;
        assembly {
            ms.slot := position
        }
    }
