...
describe("Did Documents", () => {
  ...
  it("should insertDidDocument", async () => {
    const tx = reg.insertDidDocument(
      did,
      baseDocument,
      vMethodId,
      user.publicKey,
    ...
    await Promise.all([
      expect(tx).to.emit(reg, "DidDocumentInserted"),
      expect(tx).to.emit(reg, "ControllerAdded"),
      expect(tx).to.emit(reg, "VerificationRelationshipAdded"),
    ]);
  });
  ...
  it("should revoke a verification method", async () => {
    ...
    const tx = reg.revokeVerificationMethod(did, vMethodId, 3000);
    await Promise.all([
      expect(tx).to.emit(reg, "VerificationRelationshipUpdated"),
      expect(tx).to.emit(reg, "VerificationMethodRevoked"),
    ]);
  ...
  it("should expire a verification method", async () => {
    ...
    const tx = reg.expireVerificationMethod(did, vMethodId, notAfter + 3000);
    await Promise.all([expect(tx).to.emit(reg, "VerificationMethodRevoked")]);
  ...
  it("should roll a verification method", async () => {
    ...
    const tx = reg.rollVerificationMethod(
      rollArgs(
        did,
        vMethodId2,
        publicKey2,
    ...
    await Promise.all([
      expect(tx).to.emit(reg, "VerificationMethodRolled"),
      expect(tx).to.emit(reg, "VerificationMethodAdded"),
      expect(tx).to.emit(reg, "VerificationRelationshipUpdated"),
      expect(tx).to.emit(reg, "VerificationRelationshipAdded"),
    ]);
