...
describe("Policy", () => {
  ...
  describe("Get functions", () => {
    it("should fail to initialize", async () => {
      await expect(policyContract.initialize()).to.be.revertedWithCustomError(
        policyContract,
    ...
    it("Should fail for invalid policy", async () => {
      await expect(
        policyContract["getPolicy(uint256)"](5),
      ).to.be.revertedWithCustomError(policyContract, "InvalidPolicy");
    });
    ...
    it("should fail to initialize twice", async () => {
      const emptyPolicyContract = await upgrades.deployProxy(
    ...
    it("should check pagination conditions are working", async () => {
      await expect(
        policyContract.getPolicyNames(1, 51),
      ).to.be.revertedWithCustomError(policyContract, "PageSizeTooLarge");
      await expect(
        policyContract.getPolicyNames(1, 0),
      ).to.be.revertedWithCustomError(policyContract, "PageSizeZero");
      await expect(
        policyContract.getPolicyNames(0, 10),
      ).to.be.revertedWithCustomError(policyContract, "PageZero");

      await expect(
        policyContract.getPolicies(1, 51),
      ).to.be.revertedWithCustomError(policyContract, "PageSizeTooLarge");
      await expect(
        policyContract.getPolicies(1, 0),
      ).to.be.revertedWithCustomError(policyContract, "PageSizeZero");
      await expect(
        policyContract.getPolicies(0, 10),
      ).to.be.revertedWithCustomError(policyContract, "PageZero");
    });
  ...
  describe("getPolicyNamesByIds", () => {
    it("Should return names for valid policy IDs in order", async () => {
      const names = await policyContract.getPolicyNamesByIds([1n, 3n, 2n]);
      expect(names).to.deep.equal(["policy-0", "policy-2", "policy-1"]);
    });

    it("Should return single name for single ID", async () => {
      const names = await policyContract.getPolicyNamesByIds([4n]);
      expect(names).to.deep.equal(["policy-3"]);
    });

    it("Should return empty array for empty input", async () => {
      const names = await policyContract.getPolicyNamesByIds([]);
      expect(names).to.deep.equal([]);
    });

    it("Should revert for invalid policy ID (zero)", async () => {
      await expect(
        policyContract.getPolicyNamesByIds([1n, 0n, 2n]),
      ).to.be.revertedWithCustomError(policyContract, "InvalidPolicy");
    });

    it("Should revert for invalid policy ID (out of range)", async () => {
      await expect(
        policyContract.getPolicyNamesByIds([1n, 99n]),
      ).to.be.revertedWithCustomError(policyContract, "InvalidPolicy");
    });
  });
  ...
  describe("deactivatePolicy", () => {
    ...
    for (const { invalidValue, override, type, value } of tests) {
      ...
      it(`Should fail for missing policy (${type})`, async () => {
        ...
        const tx = deactivatePolicy(invalidValue);
        await (override === "string"
      ...
      it(`Should fail for inactive policy (${type})`, async () => {
        ...
        // @ts-expect-error Mismatch of types
        await expect(deactivatePolicy(value)).to.be.revertedWithCustomError(
          policyContract,
          "InvalidPolicy",
        );
      });
  ...
  describe("activatePolicy", () => {
    ...
    for (const { invalidValue, override, type, value } of tests) {
      ...
      it(`Should fail for missing policy (${type})`, async () => {
        // @ts-expect-error Mismatch of types
        const tx = activatePolicy(invalidValue);
        await (override === "string"
          ? expect(tx).to.be.reverted
          : expect(tx).to.be.revertedWithCustomError(
              policyContract,
      ...
      it(`Should fail for active policy (${type})`, async () => {
        // @ts-expect-error Mismatch of types
        await expect(activatePolicy(value)).to.be.revertedWithCustomError(
          policyContract,
          "PolicyAlreadyActive",
        );
      });
  ...
  describe("insertPolicy", () => {
    it("Should fail for empty name", async () => {
      ...
      ).to.be.revertedWithCustomError(policyContract, "PolicyNameRequired");
    });

    it("Should fail for empty description", async () => {
      await expect(
        policyContract.insertPolicy("name", ""),
      ).to.be.revertedWithCustomError(policyContract, "DescriptionRequired");
    });

    it("Should fail for same policyName", async () => {
      await expect(
        policyContract.insertPolicy("policy-1", "description"),
      ).to.be.revertedWithCustomError(policyContract, "PolicyExists");
    });
  ...
  describe("Access Control and registry", () => {
    ...
    it("UUPS proxy is deployed correctly", async () => {
      const address = await policyContract.getAddress();
  ...
  describe("Update Policy", () => {
    it("should fail to update a policy description with empty string", async () => {
      ...
      ).to.be.revertedWithCustomError(policyContract, "InvalidDescription");
    });
    ...
    it("should fail to update a policy that is inactive", async () => {
      ...
      ).to.be.revertedWithCustomError(policyContract, "PolicyInactive");
    });
    ...
    it("should fail to update policy if it doesn't have operator role", async () => {
      ...
      await expect(
        ...
        .withArgs(await addr1.getAddress(), operatorRole);
      await expect(
        policyContract
          .connect(addr1)
