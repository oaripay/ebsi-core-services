...
describe("SmartContracts Module", () => {
  ...
  describe("GET /smart-contracts/{smartContractInfoId}", () => {
    ...
    it("should throw an error if the smart contract is not found", async () => {
      ...
      const smartContractInfoId = crypto.randomBytes(32).toString("hex");
      const response = await request(server).get(
        `/smart-contracts/0x${smartContractInfoId}`
      );

      expect(response.body).toStrictEqual({
        title: "Smart Contract Not Found",
        status: 404,
        detail: `Smart contract 0x${smartContractInfoId} not found`,
        type: "about:blank",
      });
    ...
    it("should return a specific smart contract", async () => {
      ...
      expect(response.body).toStrictEqual(sc.smartContractInfo);
      expect(response.status).toBe(200);
      expect(
        (response.headers as { "content-type": string })["content-type"]
