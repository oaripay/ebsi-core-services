...
describe("Logging interceptor", () => {
  ...
  describe("POST /sessions with bad payload", () => {
    it("should log the request and response", async () => {
      expect.assertions(2);

      await request(app.getHttpServer()).post("/sessions").send("invalid body");
      ...
      expect(mockedLogger.log).toHaveBeenNthCalledWith(
        ...
        "LoggingInterceptor - POST - /sessions",
        "LoggingInterceptor"
      );
      ...
      expect(mockedLogger.warn).toHaveBeenNthCalledWith(
        ...
        "LoggingInterceptor - 400 - POST - /sessions",
        "LoggingInterceptor"
      );
    });
  });
