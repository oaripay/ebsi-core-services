...
- [Technical Specifications](https://ec.europa.eu/digital-building-blocks/wikis/x/koXkFQ)
- API catalogs:
...
- [Users onboarding API](#users-onboarding-api)
...
You must at least set `API_PRIVATE_KEY`, `API_NAME`, `API_VERIFICATION_METHOD_KID`, `EU_LOGIN_VALIDATE_SERVICE_URL`, and `EU_LOGIN_SERVICE_PARAM` to run the API. For e2e testing, you must also set `TEST_USER_PRIVATE_KEY`, `TEST_USER_KID`, `TEST_EU_LOGIN_USERNAME`, `TEST_EU_LOGIN_PASSWORD`, and `TEST_RECAPTCHA_TOKEN`.
...
Reminder: you need to set `API_PRIVATE_KEY`, `TEST_USER_PRIVATE_KEY`, and `TEST_USER_KID` (preferably in `.env.test.local`) before running the e2e tests!
Run all the tests:
