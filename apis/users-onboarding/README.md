...
You must at least set `API_PRIVATE_KEY`, `API_NAME`, `API_VERIFICATION_METHOD_KID` and `EBSI_ENV` to run the API. For e2e testing, you must also set `TEST_USER_PRIVATE_KEY`, `TEST_USER_KID`, `TEST_EU_LOGIN_USERNAME`, `TEST_EU_LOGIN_PASSWORD`, and `TEST_RECAPTCHA_TOKEN`.
