...
describe("setupInterceptors", () => {
  ...
  it("should not call the remote server if the local server responded with a status < 500", async () => {
    ...
    await expect(() =>
      axios.get("https://test.intebsi.xyz/trusted-apps-registry/v3/apps")
  ...
  it.skip("should fallback to the remote server if the local server responds with a status >= 500", async () => {
    ...
    expect(logger.verbose).toHaveBeenNthCalledWith(
      1,
      "Replacing https://test.intebsi.xyz/trusted-apps-registry/v3/apps with http://api.local/trusted-apps-registry/v3/apps",
      "Axios Request Interceptor"
    );
    expect(logger.verbose).toHaveBeenNthCalledWith(
      2,
      "Replacing http://api.local/trusted-apps-registry/v3/apps with https://test.intebsi.xyz/trusted-apps-registry/v3/apps",
      "Axios Response Interceptor"
    );
  });
