...
import { RangeNotSatisfiableError, ProblemDetailsError } from "../index.js";

describe("error RangeNotSatisfiableError", () => {
  it("should extend ProblemDetailsError", () => {
    expect.assertions(1);

    const error = new RangeNotSatisfiableError("Test Error");

    expect(error instanceof ProblemDetailsError).toBe(true);
  });

  it("should have 'Range Not Satisfiable' as default title", () => {
    expect.assertions(1);

    const error = new RangeNotSatisfiableError();

    expect(error.title).toBe("Range Not Satisfiable");
  });
  ...
  it("should have the correct properties", () => {
    ...
    expect(error.name).toBe("RangeNotSatisfiableError");
    expect(error.status).toStrictEqual(RangeNotSatisfiableError.statusCode);
  ...
  it("should print the correct JSON object", () => {
    ...
    expect(jsonError).toStrictEqual({
      custom: "value",
      status: RangeNotSatisfiableError.statusCode,
      title: "Test Error",
      type: "test",
    });
  });
  ...
  it("should print the correct string", () => {
    ...
    expect(error.toString()).toBe(
      `${RangeNotSatisfiableError.statusCode} - Test Error`,
    );
  });
});
