...
import { LockedError, ProblemDetailsError } from "../index";

describe("error LockedError", () => {
  it("should extend ProblemDetailsError", () => {
    expect.assertions(1);

    const error = new LockedError("Test Error");

    expect(error instanceof ProblemDetailsError).toBe(true);
  });

  it("should have 'Locked' as default title", () => {
    expect.assertions(1);

    const error = new LockedError();

    expect(error.title).toBe("Locked");
  });
  ...
  it("should have the correct properties", () => {
    ...
    expect(error.status).toStrictEqual(LockedError.statusCode);
  ...
  it("should print the correct string", () => {
    ...
    expect(error.toString()).toBe(`${LockedError.statusCode} - Test Error`);
  });
});
