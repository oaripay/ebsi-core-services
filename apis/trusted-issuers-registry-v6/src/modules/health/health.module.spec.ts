...
describe("Health Module", () => {
  ...
  describe("GET /health", () => {
    ...
    it("should return 'error' if the Subgraph is not synced", async () => {
      ...
      // Expect all the runtime dependencies to be up except TIR Subgraph
      const expectedStatuses = {
        ...dependencies
          .map((dependency) => ({
            [`${dependency}@${RUNTIME_DEPENDENCIES[dependency]}`]: {
      ...
      const { "TIR Subgraph": errorStatus, ...otherStatuses } =
        expectedStatuses;

      expect(response.body).toStrictEqual({
        details: expectedStatuses,
        error: {
          "TIR Subgraph": errorStatus,
        },
        info: otherStatuses,
        status: "error",
      });
