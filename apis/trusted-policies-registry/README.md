...
You must at least set `API_NAME`, `API_PRIVATE_KEY`, `CONTRACT_ADDR`, and `EBSI_ENV` to run the API.
...
Reminder: you need to set `API_PRIVATE_KEY`, `TEST_USER_KID`, `TEST_USER_PRIVATE_KEY`, `TEST_ADMIN_KID` and `TEST_ADMIN_PRIVATE_KEY` (preferably in `.env.test.local`) before running the e2e tests!
