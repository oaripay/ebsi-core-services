...
export class AuthService {
  ...
  async validateToken(
    ...
    // The Access Token `scp` must contain one of the following scopes:
    // - "tir_invite"
    // - "tir_write"
    if (!scp.includes("tir_invite") && !scp.includes("tir_write")) {
      throw new UnauthorizedError(UnauthorizedError.defaultTitle, {
        detail: "Invalid JWT: scp must contain tir_invite or tir_write",
      });
    }
