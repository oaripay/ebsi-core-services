...
import { metadata as attestationSchemaMetadata1 } from "@cef-ebsi/vcdm1.1-attestation-schema";
import { metadata as attestationSchemaMetadata2 } from "@cef-ebsi/vcdm2.0-attestation-schema";
...
describe.each(["1.1", "2.0"] as const)(
  "Authorisation Module (using VCDM %s)",
  (vcdmVersion) => {
    describe.each(["EBSI URI", "URL"] as const)(
      ...
      (uriType) => {
        ...
        const testOidSchemaUrl = `https://api-test.ebsi.eu/trusted-schemas-registry/v3/schemas/${
          vcdmVersion === "1.1"
            ? attestationSchemaMetadata1.id.multibase_base58btc
            : attestationSchemaMetadata2.id.multibase_base58btc
        }`;
        ...
        describe("GET /presentation-definitions", () => {
          ...
          it("should return the expected presentation definition for the given scope", async () => {
            ...
            expect(response.body).toStrictEqual(
              tntCreatePresentationDefinition,
            );
            expect(response.status).toBe(200);
                ...
                async function setupEnvironment() {
                  ...
                  vcPayload =
                    vcdmVersion === "1.1"
                      ...
                      : {
                          ...
                          validFrom: `${issuanceDate.toISOString().slice(0, -5)}Z`,
                          validUntil: `${expirationDate.toISOString().slice(0, -5)}Z`,
                        };
                  ...
                  switch (customScope) {
                    ...
                    case TIR_INVITE_SCOPE: {
                      if (vcdmVersion === "2.0")
                        vcPayload.type.push("VerifiableTrustModel");
...
- Validation error. Path: 'presentation_submission.id'. Reason: Required`,
                  ...
                  invalidPresentationSubmission.definition_id =
                    "invalid_def_id";
                  response = await request(server)
                    .post("/token")
                    .set("Content-Type", "application/x-www-form-urlencoded")
