...
describe.each(["fixed", "deprecated"] as const)(
  "TSR API v3 - Schemas (e2e, %s schema IDs)",
  (schemaIdType) => {
    ...
    beforeAll(async () => {
      ...
      if (writeOps()) {
        ...
        const testAdminPrivateKeyHex = configService.get(
          "testAdminPrivateKey",
          {
            infer: true,
          },
        ...
        adminTestWallet = new ethers.Wallet(
          prefixWith0x(
            configService.get("testAdminPrivateKey", { infer: true }),
          ),
        );
      ...
      const schemaIdBuffer =
        schemaIdType === "fixed"
          ...
          : await computeId__deprecated(rawSchema);
    ...
    describe("GET /schemas/{schemaId}/revisions", () => {
      ...
      describeWriteOps()("Test requiring actual data", () => {
        it("should return the revisions of the specified schema", async () => {
          ...
          const revisionId2 = ethers.sha256(
