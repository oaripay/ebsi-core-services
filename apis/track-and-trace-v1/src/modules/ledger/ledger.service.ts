...
export class LedgerService {
  ...
  private trackAndTraceContract?: TrackAndTrace;
  ...
  private async getPublicMethodsTrackAndTraceContract() {
    if (this.publicMethodsTrackAndTraceContract) {
      return this.publicMethodsTrackAndTraceContract;
    }
    ...
    return this.publicMethodsTrackAndTraceContract as unknown as TrackAndTrace;
  }

  async getContract({ protectedMethod = false } = {}): Promise<TrackAndTrace> {
    if (protectedMethod) {
      await this.checkSession();
      return this.trackAndTraceContract as TrackAndTrace; // Assume it is not undefined
    }

    return this.getPublicMethodsTrackAndTraceContract();
  }
