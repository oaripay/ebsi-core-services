...
describe("Schemas Module", () => {
      ...
      expect(response.body).toStrictEqual({
        items: expect.arrayContaining(
          dummySchemas.map((schema) => {
            const schemaId = hexToMultibaseBase58Btc(schema.id);
            return {
  ...
  describe("GET /schemas/{schemaId}", () => {
    ...
    it("should return a specific schema identified by an hexadecimal schema ID", async () => {
      ...
      const schema = dummySchemas[0]!;

      const response = await request(server).get(`/schemas/${schema.id}`);

      expect(response.body).toStrictEqual(
        JSON.parse(schema.lastRevision.content),
      );
      expect(response.status).toBe(200);
      expect(
        (response.headers as { "content-type": string })["content-type"],
  ...
  describe("GET /schemas/{schemaId}/revisions", () => {
    ...
    it("should throw an error if valid-at query parameter is not valid", async () => {
      ...
      const response = await request(server).get(
        `/schemas/${schema.id}/revisions?valid-at=abc`,
      );
    ...
    it("should return the revisions of the specified schema", async () => {
      ...
      const response = await request(server).get(
        `/schemas/${schema.id}/revisions`,
      );

      const revisionId1 = schema.lastRevision.id;
      const revisionId2 = schemaRevisions[0]!.id;
      const revisionId3 = schemaRevisions[1]!.id;

      expect(response.body).toStrictEqual({
        items: expect.arrayContaining([
          {
            href: expect.stringContaining(
              `/schemas/${schema.id}/revisions/${revisionId1}`,
            ),
            schemaRevisionId: revisionId1,
          },
          {
            href: expect.stringContaining(
              `/schemas/${schema.id}/revisions/${revisionId2}`,
            ),
            schemaRevisionId: revisionId2,
          },
          {
            href: expect.stringContaining(
              `/schemas/${schema.id}/revisions/${revisionId3}`,
            ),
        ...
        links: {
          first: expect.stringContaining(
            `/schemas/${schema.id}/revisions?page[after]=1&page[size]=10`,
          ),
          last: expect.stringContaining(
            `/schemas/${schema.id}/revisions?page[after]=1&page[size]=10`,
          ),
    ...
    it("should return the revisions valid at a specific date", async () => {
      ...
      const response = await request(server).get(
        `/schemas/${schema.id}/revisions?valid-at=${validAt}`,
      );

      const revisionId1 = schema.lastRevision.id;
      const revisionId2 = schemaRevisions[0]!.id;
      const revisionId3 = schemaRevisions[1]!.id;
      const selfLink = `/schemas/${schema.id}/revisions?page[after]=1&page[size]=10&valid-at=${encodeURIComponent(validAt)}`;
    ...
    it("should handle the pagination properly", async () => {
      ...
      const response1 = await request(server).get(
        `/schemas/${schema.id}/revisions?page[size]=2`,
      );
      expect(response1.body).toStrictEqual({
        items: expect.arrayContaining([]),
        links: {
          first: expect.stringContaining(
            `/schemas/${schema.id}/revisions?page[after]=1&page[size]=2`,
          ),
          next: expect.stringContaining(
            `/schemas/${schema.id}/revisions?page[after]=2&page[size]=2`,
          ),
          prev: expect.stringContaining(
            `/schemas/${schema.id}/revisions?page[after]=1&page[size]=2`,
          ),
      ...
      // big page
      const response3 = await request(server).get(
        `/schemas/${schema.id}/revisions?page[after]=100&page[size]=2`,
      );
      expect(response3.body).toStrictEqual({
        items: expect.arrayContaining([]),
        links: {
          ...
          prev: expect.stringContaining(
            `/schemas/${schema.id}/revisions?page[after]=99&page[size]=2`,
          ),
        },
        pageSize: 2,
        self: expect.stringContaining(
      ...
      // page["after"] defined but page["size"] undefined
      const response4 = await request(server).get(
        `/schemas/${schema.id}/revisions?page[after]=1`,
      );
      expect(response4.body).toStrictEqual({
        items: expect.arrayContaining([]),
        links: {
  ...
  describe("GET /schemas/{schemaId}/revisions/{schemaRevisionId}", () => {
    ...
    it("should return a specific schema revision", async () => {
      ...
      const schema = dummySchemas[0]!;
      const { id: schemaId } = schema;
      const schemaRevisionId = schema.lastRevision.id;
  ...
  describe("GET /schemas/{schemaId}/revisions/{schemaRevisionId}/metadata", () => {
    ...
    it("should return the metadata of the specified schema revision", async () => {
      ...
      const schema = dummySchemas[0]!;
      const revisionId = schema.lastRevision.id;

      const response = await request(server).get(
        `/schemas/${schema.id}/revisions/${revisionId}/metadata`,
      );
      ...
      expect(response.body).toStrictEqual({
        ...
        links: {
          first: expect.stringContaining(
            `/schemas/${schema.id}/revisions/${revisionId}/metadata?page[after]=1&page[size]=10`,
          ),
          last: expect.stringContaining(
            `/schemas/${schema.id}/revisions/${revisionId}/metadata?page[after]=1&page[size]=10`,
          ),
    ...
    it("should handle the pagination properly", async () => {
      ...
      const response1 = await request(server).get(
        `/schemas/${schema.id}/revisions/${revisionId}/metadata?page[size]=2`,
      );
      expect(response1.body).toStrictEqual({
        items: expect.arrayContaining([]),
        links: {
          first: expect.stringContaining(
            `/schemas/${schema.id}/revisions/${revisionId}/metadata?page[after]=1&page[size]=2`,
          ),
          next: expect.stringContaining(
            `/schemas/${schema.id}/revisions/${revisionId}/metadata?page[after]=2&page[size]=2`,
          ),
          prev: expect.stringContaining(
            `/schemas/${schema.id}/revisions/${revisionId}/metadata?page[after]=1&page[size]=2`,
          ),
      ...
      // big page
      const response3 = await request(server).get(
        `/schemas/${schema.id}/revisions/${revisionId}/metadata?page[after]=100&page[size]=2`,
      );
      expect(response3.body).toStrictEqual({
        items: expect.arrayContaining([]),
        links: {
          ...
          prev: expect.stringContaining(
            `/schemas/${schema.id}/revisions/${revisionId}/metadata?page[after]=99&page[size]=2`,
          ),
        },
        pageSize: 2,
        self: expect.stringContaining(
      ...
      // page["after"] defined but page["size"] undefined
      const response4 = await request(server).get(
        `/schemas/${schema.id}/revisions/${revisionId}/metadata?page[after]=1`,
      );
      expect(response4.body).toStrictEqual({
        items: expect.arrayContaining([]),
        links: {
  ...
  describe("GET /schemas/{schemaId}/revisions/{schemaRevisionId}/metadata/{metadataId}", () => {
    ...
    it("should return the expected metadata", async () => {
      ...
      const { id: schemaId } = schema;
      const schemaRevisionId = schema.lastRevision.id;
      const metadataId = schema.lastRevision.metadata[0]!.id;
      ...
      expect(response.body).toStrictEqual(
        JSON.parse(schema.lastRevision.metadata[0]!.content),
      );
      expect(response.status).toBe(200);
      expect(
        (response.headers as { "content-type": string })["content-type"],
