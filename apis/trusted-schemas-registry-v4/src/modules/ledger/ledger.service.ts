...
export class LedgerService {
  ...
  private tsrContract: TrustedSchemasRegistry | undefined;

  private publicMethodsTsrContract: TrustedSchemasRegistry | undefined;
  ...
  private async refreshConnection() {
    if (this.privateProviderMutex.isLocked()) {
      ...
      await this.privateProviderMutex.runExclusive(async () => {
        ...
        this.tsrContract = TrustedSchemasRegistry__factory.connect(
          this.tsrAddress,
          provider,
        );
      });
  ...
  private async getPublicMethodsTsrContract() {
    ...
    if (this.publicProviderMutex.isLocked()) {
      ...
      await this.publicProviderMutex.runExclusive(async () => {
        ...
        this.publicMethodsTsrContract = TrustedSchemasRegistry__factory.connect(
          this.tsrAddress,
          provider,
        );
      });
  ...
  } = {}): Promise<TrustedSchemasRegistry> {
    if (protectedMethod) {
