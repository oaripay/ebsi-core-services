...
export const handlers = [
  ...
  graphql.query("GetDidsByController", ({ variables }) => {
    const { controller, pagesize, skip } = variables;
  ...
  graphql.query("GetDidsByVerificationRelationship", ({ variables }) => {
    const { pagesize, skip, vMethodId, vRelationship } = variables;
  ...
  graphql.query("GetDidDocumentByTimestamp", ({ variables }) => {
    ...
    if (did === did1) {
      const didInvalidBaseDoc = structuredClone(didDocumentData);
    ...
    if (did === did3) {
      const didInvalidKey = JSON.parse(
        JSON.stringify(didDocumentData).replaceAll(new RegExp(did2, "g"), did3),
      );
