...
You must at least set `API_PRIVATE_KEY`, `CONTRACT_ADDR`, and `DOMAIN` to run the API. For e2e testing, you must also set `TEST_CLIENT_KID` and `TEST_CLIENT_PRIVATE_KEY`.
...
You can now open http://localhost:3000/did-registry/v3/health. If everything's working correctly, then you should see `"status":"ok"`.
...
Check http://localhost:3000/did-registry/v3/health to see if it's working.
