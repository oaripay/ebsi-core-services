...
You must at least set `API_NAME`, `API_PRIVATE_KEY`, `CONTRACT_ADDR` and `EBSI_ENV` to run the API. For e2e testing, you must also set `TEST_ADMIN_PRIVATE_KEY` and `TEST_ADMIN_KID`.
...
You can now open http://localhost:3000/trusted-schemas-registry/v2/health. If everything's working correctly, then you should see `"status":"ok"`.
...
Check http://localhost:3000/trusted-schemas-registry/v2/health to see if it's working.
...
Reminder: you need to set `TEST_ADMIN_PRIVATE_KEY` and `TEST_ADMIN_KID` (preferably in `.env.test.local`) before running the e2e tests!
