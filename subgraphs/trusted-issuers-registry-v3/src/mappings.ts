...
export function handleAddIssuerProxyEvent(event: AddIssuerProxy): void {
  ...
  const proxy = new Proxy(event.params.proxyId);
...
export function handleUpdateIssuerProxyEvent(event: UpdateIssuerProxy): void {
  ...
  const proxy = Proxy.load(event.params.proxyId);
...
function handleSetAttributeData(event: AddAttributeRevision): void {
  ...
  // Load attribute
  const attribute = Attribute.load(event.params.attributeId);
  ...
  const latestRevision = Revision.load(attribute.latestRevision);
  ...
  // Create new revision
  const newRevision = new Revision(event.params.revisionId);
...
function handleSetAttributeMetadata(event: AddAttributeRevision): void {
  ...
  // Load attribute
  let attribute = Attribute.load(event.params.attributeId);
  ...
  if (attribute) {
    ...
    attribute = new Attribute(event.params.attributeId);
  ...
  // Create new revision
  const revision = new Revision(event.params.revisionId);
  ...
  if (issuerType === ROOT_TAO) {
    ...
    const taoAttributeRevision = Revision.load(taoAttributeId);
    ...
    const taoAttribute = Attribute.load(taoAttributeRevision.attribute);
    ...
    const taoAttributeLatestRevision = Revision.load(
