...
export function handleTimestampedHashes(event: TimestampedHashes): void {
  for (let i = 0; i < event.params.hashAlgorithmIds.length; i += 1) {
    ...
    if (i < event.params.timestampData.length) {
      timestamp.timestampData = event.params.timestampData[i];
    } else {
      timestamp.timestampData = new Bytes(0);
    }
    timestamp.save();
  }
}
...
export function handleNewRecord(event: NewRecord): void {
  ...
  for (let i = 0; i < event.params.timestampIds.length; i += 1) {
