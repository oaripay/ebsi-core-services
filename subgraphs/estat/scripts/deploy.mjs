...
$$`${graphBinary} codegen`;
$$`${graphBinary} build`;
$$`${graphBinary} create ebsi/estat --node ${process.env["NODE_URL"]}`;
$$`${graphBinary} deploy ebsi/estat --version-label ${version} --node ${process.env["NODE_URL"]} --ipfs ${process.env["IPFS_URL"]}`;
