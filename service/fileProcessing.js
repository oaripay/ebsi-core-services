...
async function login() {
    ...
    // };
    // var credential = await new Web3().eth.accounts.sign(JSON.stringify(message), config.private_key);
...
function upload(req, res) {
    ...
    //     console.log('------------------------------------>>>>>> ', JSON.stringify(req.session.ecas_session));
    // }
    ...
    sampleFile.mv(fileToStore, function(err) {
        ...
        Promise.all([
            ...
            if (response[0] && response[0].ok) {
                // console.log(' file stored ::: ', response[0].ok, ' | ', response[0].message);
                res.render('index', { title: 'Notary DApp', message: response[0].message, hash: response[0].hash, ok: response[0].ok, user: response[0].user, notary: true, allDocument: response[1].data, 'transactionId': response[0].transactionId });
            } else {
                // let allDocument = [];
                // if(response && response[1] && response.data[1]) allDocument = response[1].data;
                // res.render('index', { title: 'Notary DApp', message: response[0].message, ok: response[0].ok, user: response[0].user, allDocument: allDocument });
                // res.render('index', { title: 'Notary DApp', message: response[0].message, ok: response[0].ok, user: response[0].user, allDocument: response[1].data });
                res.render('index', { title: 'Notary DApp', message: response[0].message, ok: response[0].ok, user: response[0].user, allDocument: response[1].data });
            }
...
async function storeDocWithoutPubKey(username) {
    ...
    //var pubKey = '0xfaB57D6F7182A98fEF9d23A18c6c607f46f4fC94';

    // console.log('== _-_--- storeDocWithoutPubKey ---_-_ ==\n', username);
    ...
    if (filename) {
        ...
        try {
            ...
            // console.log('storeOpts: ', storeOpts);
...
async function signIt(hash, token) {
    ...
    var query = { jsonrpc: '2.0', method: 'eth_sendRawTransaction', params: [signed.rawTransaction], id: 1 }
    var opts = { headers: { "Authorization": `Bearer ${token}` } }
    var response = await axios.post(rpc_node, query, opts)
    console.log('-----------------------------------------------\n response.data: \n', response.data);
    // console.log('-----------------------------------------------\n response: \n', response)
    return response;
...
async function getNotarizedDocument(txHash, username) {
    ...
    try {
        ...
        // can be implemented using contract so...
        // var timestamp = await contract.methods.timestamp(hash).call();
